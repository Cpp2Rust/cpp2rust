...
unsafe fn main_0() -> i32 {
    let mut a: i32 = 42;
    (unsafe { run_2(Some(negate_0 as unsafe fn(*mut i32)), (&mut a as *mut i32)) });
    assert!(((a) == (-42_i32)));
    (unsafe {
        run_2(
            Some(zero_out_1 as unsafe fn(*mut i32)),
            (&mut a as *mut i32),
        )
    });
    assert!(((a) == (0)));
    let mut fn_: Option<unsafe fn(*mut i32)> = Some(negate_0 as unsafe fn(*mut i32));
    assert!(!((fn_).is_none()));
    let mut b: i32 = 10;
