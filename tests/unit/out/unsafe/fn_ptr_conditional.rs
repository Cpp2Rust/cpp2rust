...
pub unsafe fn pick_3(mut mode: i32) -> Option<unsafe fn(i32) -> i32> {
    return if ((mode) > (0)) {
        Some(inc_0 as unsafe fn(i32) -> i32)
    } else {
        if ((mode) < (0)) {
            Some(dec_1 as unsafe fn(i32) -> i32)
        } else {
            Some(identity_2 as unsafe fn(i32) -> i32)
        }
    };
}
...
unsafe fn main_0() -> i32 {
    ...
    assert!(((unsafe { apply_4(Some(inc_0 as unsafe fn(i32) -> i32), 5,) }) == (6)));
