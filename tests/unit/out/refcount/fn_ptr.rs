...
pub fn foo_1(fn_: Ptr<fn(AnyPtr) -> i32>, pi: Ptr<i32>) -> i32 {
    let fn_: Value<Ptr<fn(AnyPtr) -> i32>> = Rc::new(RefCell::new(fn_));
...
fn main_0() -> i32 {
    let fn_: Value<Ptr<fn(AnyPtr) -> i32>> = Rc::new(RefCell::new(Ptr::null()));
    ...
    assert!({
        let _lhs = ({
            let _fn: Ptr<fn(AnyPtr) -> i32> = (*fn_.borrow()).clone();
