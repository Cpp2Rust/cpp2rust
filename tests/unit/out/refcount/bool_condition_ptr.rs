...
fn main_0() -> i32 {
    ...
    assert!(((*t4.borrow()) == 0));
    let t5: Value<i32> = Rc::new(RefCell::new(((!(!(*p.borrow()).is_null()) as bool) as i32)));
    assert!(((*t5.borrow()) == 0));
    let t6: Value<i32> = Rc::new(RefCell::new(
        ((!(!(*np.borrow()).is_null()) as bool) as i32),
    ));
    assert!(((*t6.borrow()) == 1));
    let b2: Value<bool> = Rc::new(RefCell::new((!(*p.borrow()).is_null()).clone()));
    let b3: Value<bool> = Rc::new(RefCell::new((!(*np.borrow()).is_null()).clone()));
    assert!((*b2.borrow()));
    assert!((!(*b3.borrow()) as bool));
    return 0;
}
