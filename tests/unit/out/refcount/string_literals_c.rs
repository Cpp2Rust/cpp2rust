...
fn main_0() -> i32 {
    let mutable_strings: Value<Box<[Ptr<core::ffi::c_char>]>> = Rc::new(RefCell::new(Box::new([
    ...
    let mutable_string: Value<Ptr<core::ffi::c_char>> =
    ...
    let immutable_string_arr: Value<Box<[core::ffi::c_char]>> =
        Rc::new(RefCell::new(Box::from(libcc2rs::char_array(b"papanasi\0"))));
    let mutable_empty: Value<Ptr<core::ffi::c_char>> =
    ...
    ({
        let _str: Ptr<core::ffi::c_char> = (*mutable_string.borrow()).clone();
        foo_mut_0(_str)
    });
    ({
    ...
    ({
        let _str: Ptr<core::ffi::c_char> = (*mutable_empty.borrow()).clone();
        foo_const_1(_str)
    });
    ({
    ...
    ({
        let _str: Ptr<core::ffi::c_char> =
            (mutable_empty_arr.as_pointer() as Ptr<core::ffi::c_char>);
        foo_const_1(_str)
    });
    ({
        let _str: Ptr<core::ffi::c_char> =
