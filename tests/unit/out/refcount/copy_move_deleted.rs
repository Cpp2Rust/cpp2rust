...
impl Container {
    pub fn Container_pmutContainer(_a0: Ptr<Container>) -> Self {
        let __this: Value<Container> = Rc::new(RefCell::new(Self {
            inner: Rc::new(RefCell::new(NoCopy::NoCopy_pmutNoCopy({
...
fn main_0() -> i32 {
    ...
    let d: Value<Container> = Rc::new(RefCell::new(Container::Container_pmutContainer({
        c.as_pointer()
    })));
    assert!(
...
pub trait ContainerImpl {
    fn operator_assign_pmutContainer(&self, _a0: Ptr<Container>) -> Ptr<Container>;
}
impl ContainerImpl for Ptr<Container> {
    fn operator_assign_pmutContainer(&self, _a0: Ptr<Container>) -> Ptr<Container> {
        ({
            let _o: Ptr<NoCopy> = (*_a0.upgrade().deref()).inner.as_pointer();
            NoCopyImpl::operator_assign_pmutNoCopy(
