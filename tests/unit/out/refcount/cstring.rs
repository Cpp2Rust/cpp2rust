...
pub fn test_memcmp_2() {
    ...
    assert!(
        ((((a.as_pointer() as Ptr::<u8>) as Ptr::<u8>).to_any()).memcmp(
    ...
    assert!(
        ((((c.as_pointer() as Ptr::<u8>) as Ptr::<u8>).to_any()).memcmp(
...
pub fn test_memmove_3() {
    ...
    let r: Value<AnyPtr> = Rc::new(RefCell::new({
        (((buf.as_pointer() as Ptr<u8>).offset((1) as isize) as Ptr<u8>).to_any()).memcpy(
            &(((buf.as_pointer() as Ptr<u8>) as Ptr<u8>).to_any()),
            (4_usize) as usize,
        );
        (((buf.as_pointer() as Ptr<u8>).offset((1) as isize) as Ptr<u8>).to_any()).clone()
    }));
    assert!(
        ({
            let _lhs = (*r.borrow()).clone();
...
pub fn test_memchr_8() {
    ...
    let r: Value<AnyPtr> = Rc::new(RefCell::new({
        let mut __p =
            (((data.as_pointer() as Ptr<u8>) as Ptr<u8>).to_any()).reinterpret_cast::<u8>();
        let mut __i: usize = 0;
        loop {
            if __i == (4_usize) {
